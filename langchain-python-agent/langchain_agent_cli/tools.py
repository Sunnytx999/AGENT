...
class TodoItem(BaseModel):
    """One item in the executor's task list."""
...
def make_todo_write_tool(
    on_update: Callable[[list[TodoItem]], None],
):
    """Create a todo tool whose updates are stored by the owning agent."""

    @tool
    def todo_write(todos: list[TodoItem]) -> str:
        """Replace the current task list with the provided todos.

        Each todo must include content, status, and activeForm.
        """
        on_update(todos)
        return "Todos updated successfully. Continue using the todo list to track progress."
...
def get_local_time() -> dict[str, object]:
    """Get the current date, time, timezone, and Unix timestamp."""
...
def read_file(path: str, offset: int = 1, limit: int = MAX_READ_LINES) -> dict[str, object]:
    """Read numbered lines from a UTF-8 file."""
...
def list_files(pattern: str = "**/*") -> dict[str, object]:
    """List local files matching a glob."""
...
def write_file(path: str, content: str) -> dict[str, object]:
    """Create or completely overwrite a UTF-8 text file, creating parent directories."""
    resolved = Path(path).expanduser().resolve()
...
def edit_file(path: str, old_string: str, new_string: str, replace_all: bool = False) -> dict[str, object]:
    """Edit a UTF-8 file by exact replacement; require a unique match unless replace_all is true."""
    resolved = Path(path).expanduser().resolve()
    if not resolved.is_file():
