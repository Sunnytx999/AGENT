...
这是 `langgraph-plan-python-agent-flask` 的独立 HTTP进程间通信版本。父 Python进程只加载一次 C++ DLL，DLL创建 Flask子进程，Flask再通过本机 HTTP/JSON调用父进程中的原生工具。详细结构见 [HTTP_IPC_ARCHITECTURE.md](HTTP_IPC_ARCHITECTURE.md)。
...
不要直接运行 `python app.py`；它必须由 DLL创建，才能继承私有工具地址和认证令牌。
